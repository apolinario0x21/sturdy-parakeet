...
import { Callout, CodeBlock, StepList, type Step } from '@/components/article/ArticleBlocks';
...
export const metadata: Metadata = {
  title: 'Ambiente de Desenvolvimento Java no Linux | Marcelo Apolinário',
  description:
    'Guia técnico para instalar Java 21, Maven, IntelliJ, PostgreSQL 17, pgAdmin, Postman e Git no Ubuntu 24.04.'
};

export default function AmbienteInstalacaoJavaLinuxPage() {
  const relatedArticles = articles.filter((article) => article.slug !== '/artigos/ambiente-instalacao-java-linux').slice(0, 2);
  ...
  const articleStructuredData = {
    ...
    description:
      'Guia técnico para configurar Java 21, Maven, IntelliJ, PostgreSQL 17, pgAdmin, Postman e Git no Ubuntu 24.04.',
    author: {
      '@type': 'Person',
    ...
    <main className="mx-auto max-w-5xl px-4 py-8 text-term-text sm:px-6 lg:px-8">
      ...
      <article className="overflow-hidden rounded-2xl border border-term-border/80 bg-slate-950/70 shadow-[0_0_40px_rgba(66,255,159,0.07)]">
        ...
        <div className="space-y-8 px-4 py-6 sm:px-8 sm:py-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {stackItems.map((item) => (
                <div key={item.title} className="rounded-xl border border-term-border/70 bg-term-panel/40 px-4 py-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-term-amber">{item.label}</p>
          ...
          <section className="terminal-output">
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-term-mute">Próximos artigos</p>
            <ul className="grid gap-2 text-sm">
              {relatedArticles.map((article) => (
                <li key={article.title} className="flex items-center justify-between gap-2">
                  <span>{article.title}</span>
                  {article.available && article.slug ? (
                    <Link href={article.slug} className="text-term-cyan hover:underline">
                      Abrir
                    </Link>
                  ) : (
                    <span className="text-term-mute">Em revisão</span>
                  )}
                </li>
              ))}
            </ul>
