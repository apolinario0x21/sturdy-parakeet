...
const statusStyles = {
  WIP: 'border-term-amber/40 bg-term-amber/10 text-term-amber',
        ...
        {projects.map((project) => (
          <article key={project.name} className="terminal-output space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-[0.14em] ${statusStyles[project.status]}`}>
                {project.status}
              </span>
            </div>
            ...
            <dl className="grid gap-2 text-xs sm:text-sm">
              <div>
                <dt className="font-semibold uppercase tracking-[0.08em] text-term-cyan">Problema</dt>
                <dd className="text-term-mute">{project.problem}</dd>
              </div>
              <div>
                <dt className="font-semibold uppercase tracking-[0.08em] text-term-cyan">Resultado</dt>
            ...
            {project.href ? (
              ...
              <p className="text-sm text-term-mute">Repositório privado</p>
            )}
          </article>
        ))}
      </div>
