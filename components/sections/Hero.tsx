...
const heroStack = ['Kubernetes', 'Terraform', 'AWS/GCP', 'Go', 'Networking'];
  ...
  return (
    <a
      ...
      className={`inline-flex items-center rounded-md border px-4 py-2 text-xs font-semibold uppercase tracking-[0.12em] transition sm:text-sm ${
        primary
          ? 'border-green-600 bg-green-600/95 text-white hover:-translate-y-0.5 hover:border-term-cyan hover:bg-term-cyan hover:text-slate-950'
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-x-hidden overflow-y-auto px-3 pb-4 pt-3 text-sm leading-6 sm:px-6 sm:pb-6 sm:pt-4 sm:text-base sm:leading-7 lg:pb-8">
          <div className="flex flex-1 items-center">
            <div className="mx-auto w-full max-w-[72ch] text-center">
              ...
              <h1 className={`mt-6 text-3xl font-bold text-term-green sm:text-5xl ${done ? 'hero-reveal hero-reveal--title' : ''}`}>Marcelo Apolinário</h1>
              <p
                className={`mt-4 inline-flex rounded-lg border border-term-cyan/35 bg-term-cyan/10 px-3 py-2 text-base font-medium text-term-cyan sm:text-xl ${
                  done ? 'hero-reveal hero-reveal--role' : ''
                }`}
              >
                DevOps &amp; Networking Engineer
              </p>
              <p className={`mt-5 text-base text-slate-300 sm:text-lg ${done ? 'hero-reveal hero-reveal--subtitle' : ''}`}>
                Provisiono ambientes resilientes e escaláveis.
              </p>

              <ul className="mt-6 flex flex-wrap justify-center gap-2 text-xs sm:text-sm">
                {heroStack.map((item) => (
                  <li key={item} className="rounded border border-term-border bg-slate-950/60 px-2.5 py-1 text-term-amber">
                    {item}
                  </li>
                ))}
              </ul>
