          ...
          <div className="mx-auto flex w-full max-w-[72ch] flex-col">
            {rendered.map((block, index) => {
              ...
              if (block.type === 'output') {
                const outputClassByStep = block.step === 5 ? 'mb-6 space-y-2' : 'mb-4 space-y-1';
                const lineClassByStep =
                  block.step === 1
                    ? 'pl-2 text-base font-semibold text-term-green sm:text-xl'
                    : block.step === 3
                      ? 'pl-2 text-sm font-semibold tracking-[0.01em] text-term-cyan sm:text-base'
                      : 'pl-2 text-slate-300';
