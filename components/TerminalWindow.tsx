...
export function TerminalWindow({ title, children, className }: TerminalWindowProps) {
  return (
    <section className={`terminal-card min-w-0 overflow-hidden ${className ?? ''}`}>
      ...
      <div className="flex min-w-0 flex-1 flex-col space-y-3 p-2.5 sm:space-y-5 sm:p-5">{children}</div>
    </section>
  );
}
