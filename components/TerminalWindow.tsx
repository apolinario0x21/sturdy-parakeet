...
export function TerminalWindow({ title, children, className }: TerminalWindowProps) {
  return (
    <section className={`terminal-card grid min-w-0 grid-cols-1 overflow-hidden ${className ?? ''}`}>
      ...
      <div className="flex min-w-0 flex-col gap-3 p-2.5 sm:gap-5 sm:p-5">{children}</div>
    </section>
  );
}
